        ...
        <table class="w-full whitespace-no-wrap">
            <thead>
                <tr
                    ...
                    <th class="px-4 py-3">Aktif / Non-Aktifkan</th>
                ...
                <?php foreach ($users as $user): ?>
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-3 text-sm">
                            <?= esc($user->phoneNumber) ?>
                        </td>

                        <td class="px-4 py-3 text-sm">
                            <?= esc($user->role->roleName) ?>
                        </td>

                        <td class="px-4 py-3 text-sm">
                            <span class="px-2 py-1 font-semibold leading-tight rounded-full
                                <?= $user->isActive === 1 ? 'text-green-700 bg-green-100 dark:bg-green-700 dark:text-green-100'
                        ...
                        <td class="px-4 py-3 text-xs">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" <?= (int)$user->isActive === 1 ? 'checked' : '' ?>
                                    class="toggle-switch sr-only peer" data-id="<?= $user->id ?>">
                                <div
                        ...
                        <td class="px-4 py-3 text-sm">
                            <?= esc($user->lastLogin ? date('Y-m-d H:i:s', strtotime($user->lastLogin)) : '-') ?>
                        </td>

                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="<?= base_url('staff/edit/' . $user->id) ?>"
                                ...
                                <button @click="openModalConfirm({
                                                ...
                                                description: 'Staff akan dihapus, Produk yang bersangkutan akan terdampak.',
                                                confirmAction: (data) => deleteUser(data.id),
